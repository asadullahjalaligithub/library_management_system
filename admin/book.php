...
//	exit();
	...
	if($error == '')
	{
        ...
        (book_category_id, book_author_id, book_location_rack_iid, book_name, book_isbn_number, book_no_of_copy, book_status, book_added_on) 
...
if(isset($_POST["edit_book"]))
{
	...
	if($error == '')
	{
		...
		$query = "
		UPDATE lms_book 
        SET book_category_id = :book_category, 
        book_author_id = :book_author, 
        book_location_rack_iid = :book_location_rack, 
        book_name = :book_name, 
...
$query = "
	...
	inner join lms_location_rack on lms_location_rack.location_rack_id=lms_book.book_location_rack_iid
	inner join lms_author on lms_author.author_id=lms_book.book_author_id
    ORDER BY lms_book.book_id DESC
";
	...
	<?php 
		...
		else if($_GET["action"] == 'edit')
		{
			...
			if($book_id > 0)
			{
				...
				SELECT * FROM lms_book 
	inner join lms_category on lms_category.category_id=lms_book.book_category_id
	inner join lms_location_rack on lms_location_rack.location_rack_id=lms_book.book_category_id
	inner join lms_author on lms_author.author_id=lms_book.book_author_id
                WHERE lms_book.book_id = '$book_id'
				";
    ...
    <div class="card mb-4">
    	<div class="card-header">
       	...
       	<div class="card-body">
       		<form method="post">
       			<div class="row">
       				...
       				<div class="col-md-6">
       					<div class="mb-3">
       						<label class="form-label">Select Author</label>
       						<select name="book_author" id="book_author2" class="form-control">
       							<?php echo fill_author($connect); ?>
       						</select>
       					</div>
       				</div>
       			...
       			<div class="row">
       				<div class="col-md-6">
       					<div class="mb-3">
       						<label class="form-label">Select Category</label>
       						<select name="book_category" id="book_category2" class="form-control">
       							<?php echo fill_category($connect); ?>
       						</select>
       					</div>
       				</div>
       				<div class="col-md-6">
       					<div class="mb-3">
       						<label class="form-label">Select Location Rack</label>
       						<select name="book_location_rack" id="book_location_rack2" class="form-control">
       							<?php echo fill_location_rack($connect); ?>
       						</select>
       					</div>
       				</div>
       		...
       		<script>
       			document.getElementById('book_author2').value = "<?php echo $book_row['book_author_id']; ?>";
       			document.getElementById('book_category2').value = "<?php echo $book_row['book_category_id']; ?>";
       			document.getElementById('book_location_rack2').value = "<?php echo $book_row['book_location_rack_iid']; ?>";
       		</script>
       	</div>
   	</div>
	<?php
