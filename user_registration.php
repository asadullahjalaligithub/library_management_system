...
if(isset($_POST["register_button"]))
{
	...
	if(!empty($_FILES['user_profile']['name']))
	{
		...
		if(in_array($img_ext, $extensions))
		{
			if($image_size <= 2000000)
			{
				...
				{
					$message .= '<li>Image dimension should be within 225 X 225</li>'.$width." ".$height;
				}
			}
			else
			{
	...
	if($message == '')
	{
		...
		else
		{
			...
			$mail->Username = 'asadullah.jalali2013@gmail.com';

			$mail->Password = 'DepressionDepression';

			$mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;

			$mail->Port = 587;

			$mail->setFrom('asadullah.jalali2013@gmail.com', 'Webslesson');
